...
#if LV_USE_LABEL == 0
#error \
    "lv_ddlist: lv_label is required. Enable it in lv_conf.h (LV_USE_LABEL 1)"
#endif
...
/**
 * Add an options to a drop down list from a string.  Only works for dynamic
 * options.
 * @param ddlist pointer to drop down list object
 * @param option a string without '\n'. E.g. "Four"
 * @param pos the insert position, indexed from 0, LV_DROPDOWN_POS_LAST = end of
 * string
 */
...
/**
 * Set whether the ddlist highlight the last selected option and display its
 * text or not
 * @param ddlist pointer to a drop down list object
 ...
 */
void lv_dropdown_get_selected_str(const lv_obj_t* ddlist, char* buf,
                                  uint32_t buf_size);
...
/**
 * Get whether the ddlist highlight the last selected option and display its
 * text or not
 * @param ddlist pointer to a drop down list object
 * @return true/false
 */
