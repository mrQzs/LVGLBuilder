...
LVGLFontDialog::LVGLFontDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::LVGLFontDialog) {
  m_ui->setupUi(this);
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (!text.endsWith(".ttf", Qt::CaseInsensitive)) continue;
...
LVGLFontDialog::~LVGLFontDialog() { delete m_ui; }
...
void LVGLFontDialog::on_button_select_clicked() {
  QString fileName =
      QFileDialog::getOpenFileName(this, "Select font", "", "TTF Font (*.ttf)");
  if (fileName.isEmpty()) return;
  m_ui->preview->setText(fileName);
}

void LVGLFontDialog::on_list_font_currentTextChanged(
